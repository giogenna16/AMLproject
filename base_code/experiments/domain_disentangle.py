...
# 1| 0.99    | 0.0099   |   0.0001 | 0.1
# _________________________________________________
# 2| 0.99    | 0.0099   |  0.0001 | 1
# _________________________________________________
# 3| 0.0099  | 0.99     |  0.0001 | 0.01
# _________________________________________________
# 4| 0.0099  | 0.99     |  0.0001 | 0.1
# _________________________________________________
# 5| 0.99    | 0.001    |  0.009 | 0.1
...
class DomainDisentangleExperiment:  # See point 2. of the project
    ...
    def __init__(self, opt):
        ...
        # Setup of losses
        self.criterion = [torch.nn.CrossEntropyLoss(), NegHLoss(
        ), torch.nn.CrossEntropyLoss(), NegHLoss(), ReconstructionLoss()]
    ...
    def train_iteration(self, data, loss_acc_logger):
        ...
        dc_confusion_loss = W1 * \
            self.criterion[1](cat((logits1, logits2), dim=1)) * ALPHA_ENTROPY
        ...
        dom_classif_loss = W2 * \
            self.criterion[2](cat((logits1, logits2), dim=0), cat(
                (src_dom_label, tgt_dom_label), dim=0))
        ...
        c_confusion_loss = W2 * \
            self.criterion[3](cat((logits1, logits2), dim=1)) * ALPHA_ENTROPY
        ...
        reconstruction_loss = (loss4a + loss4b) / 2 * \
            W3  # is this the correct way??
        reconstruction_loss.backward()
        self.optimize_step_on_optimizers(
            ['Gen', 'Cat_Enc', 'Dom_Enc', 'Recon'])

        loss = cat_classif_loss + dc_confusion_loss + \
            dom_classif_loss + c_confusion_loss + reconstruction_loss
